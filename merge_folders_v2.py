    ...
    def interval_overlap(interval_a, interval_b):
        ...
        if x3 < x1:
            ...
            if x2 < x3:
                ...
                return min(x2, x4) - x3
    if annotation_old['shape_type'] == annotation_new['shape_type'] and annotation_old['shape_type'] == 'PolygonShape':
        p1=Polygon(annotation_old['segmentation'])
        p2=Polygon(annotation_new['segmentation'])
        return float(p1.intersection(p2).area) / max(float(p1.area + p2.area - p1.intersection(p2).area), 0.0000001)
    
    elif annotation_old['shape_type'] == annotation_new['shape_type'] and annotation_old['shape_type'] == 'RectShape':
        bbox_1 = annotation_old['bbox']
        bbox_2 = annotation_new['bbox']
        intersect_w = interval_overlap([bbox_1[0][0], bbox_1[1][0]], [bbox_2[0][0], bbox_2[1][0]])
        intersect_h = interval_overlap([bbox_1[0][1], bbox_1[1][1]], [bbox_2[0][1], bbox_2[1][1]])
...
new_images_index = {get_hash(image):image for image in new_images_filenames}
duplicates = {}
...
def tmp_func(image_path, data_src, annotations_merged, images_merged, categories_merged):
    global i
    n = 0 # счетчик номеров аннотаций
    image = get_image_by_filename(image_path.name, data_src)
    new_image = copy.deepcopy(image)
    new_image['id'] = i
    i += 1
    images_merged.append(new_image)
    for category in categories_merged:
        annotations_to_move = append_per_category(image, category, data_src)
...
for image_path in images_merged_paths:
    ...
    if get_hash(image_path) not in duplicates.keys() and get_hash(image_path) in old_images_index.keys():
        n = 0
        tmp_func(image_path, data_old, annotations_merged, images_merged, categories_merged)
                
    elif get_hash(image_path) not in duplicates.keys() and get_hash(image_path) in new_images_index.keys():
        n = 0
        tmp_func(image_path, data_new, annotations_merged, images_merged, categories_merged)
...
print("Folders merged: ", out_folder)
